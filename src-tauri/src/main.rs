...
fn log_to_file(message: &str) {
    if cfg!(debug_assertions) {
        ...
        if let Ok(mut file) = OpenOptions::new()
            ...
            .open(log_path)
        {
            let _ = writeln!(file, "{}: {}", Utc::now(), message);
        }
    }
}
            ...
            Ok((is_valid, expiration_date)) => {
                if is_valid {
                    ...
                    *license_state = true;
                    ...
                    let remaining_time = expiration_date.signed_duration_since(now).num_milliseconds();
...
#[tauri::command]
async fn check_license_validity(state: tauri::State<'_, AppState>) -> Result<serde_json::Value, String> {
    let conn = state.0.lock().unwrap();
    ...
    if let Some(license_info) = load_license_info(&conn).map_err(|e| e.to_string())? {
        ...
        match result {
            ...
            Err(e) => Ok(serde_json::json!({
                "isValid": false,
                "reason": format!("License verification failed: {}", e)
            })),
        }
    } else {
        Ok(serde_json::json!({
...
async fn auto_save_config(config: Value, config_id: i64, state: tauri::State<'_, AppState>) -> Result<(), String> {
    ...
    if let Some(current_config) = current_config {
        ...
        log_to_file(&format!("No configuration found for id: {}", config_id));
                            ...
                            match license::verify_license(&license_info.license_key, &hwid) {
                                Ok((valid, _)) => {
                                    if valid {
                                        ...
                                        let _ = window.emit("license-error", "License verification failed");
                                    }
                                },
                                Err(e) => {
                                    log_to_file(&format!("License verification error on startup: {}", e));
                                    let _ = window.emit("license-error", &format!("License error: {}", e));
                                }
                            }
                        }
                    } else {
                        log_to_file("No license found on startup");
                        let _ = window.emit("license-error", "No license found");
                    }
                }
                Ok(())
            })
