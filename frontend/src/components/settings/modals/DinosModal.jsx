...
function DinosModal({ kitName, dinos, onSave, onClose }) {
	...
	useEffect(() => {
		if (editedDinos) {
			const filtered = editedDinos.filter((dino) => {
				const searchValue = (dino[searchField] || "").toString().toLowerCase();
	...
	if (loading) return <p>Loading dino data...</p>;
	if (error) return <p>Error loading dino data: {error.toString()}</p>;
