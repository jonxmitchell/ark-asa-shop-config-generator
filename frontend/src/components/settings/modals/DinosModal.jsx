...
function DinosModal({ kitName, dinos, onSave, onClose }) {
	...
	const handleDinoChange = (index, field, value) => {
		console.log("Handling dino change:", index, field, value);
	...
	return (
		<div className="text-white" style={{ zIndex: 20 }}>
			...
			{loading && <p>Loading dino data...</p>}
			{error && <p>Error loading dino data: {error.toString()}</p>}
			{!loading && !error && (
				<div className="space-y-4 max-h-[60vh] overflow-y-auto pr-2">
					<AnimatePresence initial={false}>
						{editedDinos.map((dino, index) => (
							<motion.div
								key={index}
									...
									<div className="grid grid-cols-2 gap-4 mb-4">
										...
										<div>
											...
											<SearchableDropdown
												...
												onSelect={(selected) => {
													console.log("SearchableDropdown onSelect:", selected);
													handleDinoChange(
														index,
														"Blueprint",
														selected.Blueprint
